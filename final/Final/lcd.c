...
void lcd_rotatingBanner(char* str) {
    // init screen as empty
    char blank[] = "                    ";
    int i;

    char buffer[strlen(blank) + strlen(str) + 3];
    strcpy(buffer, blank);
    strcat(buffer, str);

    while (1) {
        // print first LCD_WIDTH chars of buffer to screen
        char screen[21]; // LCD Width + 1
        strncpy(screen, buffer, sizeof(screen) - 2);

        lcd_printf(screen);

        // rotate everything left 1
        char first = buffer[0];
        for (i = 1; i < strlen(buffer); i++) {
            buffer[i - 1] = buffer[i];
        }
        buffer[strlen(buffer) - 1] = first;
